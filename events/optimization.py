...
class RunningDinnerOptimizer:
    ...
    def load_teams(self):
        ...
        if len(self.teams) < 3:
            raise ValueError(f"Mindestens 3 Teams erforderlich, aber nur {len(self.teams)} bestätigt")
            
        logger.info(f"🎯 Optimiere {len(self.teams)} Teams für Event '{self.event.name}'")
        ...
        for i, team1 in enumerate(self.teams):
            for j, team2 in enumerate(self.teams):
                if i == j:
                    ...
                    self.distances[(team1.id, team2.id)] = self.distances[sorted_key]
                    self.distances[(team2.id, team1.id)] = self.distances[sorted_key]
    ...
    def create_mip_model(self):
                ...
                if i < ii:  # Nur eine Richtung wegen Symmetrie
                    for j, team_h in enumerate(self.teams):
                        for k, course in enumerate(self.courses):
                            ...
                            self.y[(i, ii, j, k)] = pulp.LpVariable(var_name, cat='Binary')
        ...
        for i, team_g in enumerate(self.teams):
            for ii, team_gg in enumerate(self.teams):
                if i < ii:  # Nur eine Richtung wegen Symmetrie
                    ...
                    self.z3[(i, ii)] = pulp.LpVariable(var_name, lowBound=0, cat='Integer')
        ...
        penalty_z1 = self.P1 * pulp.lpSum([self.z1[(j, k)] for j in range(n_teams) for k in range(n_courses)])
        penalty_z2 = self.P2 * pulp.lpSum([self.z2[(j, k)] for j in range(n_teams) for k in range(n_courses)])
        penalty_z3 = (self.P3 / 2) * pulp.lpSum([self.z3[(i, ii)] for i in range(n_teams) for ii in range(i+1, n_teams)])
        
        self.prob += travel_time_sum + penalty_z1 + penalty_z2 + penalty_z3
        
        logger.info("✅ MIP-Modell erstellt: Variablen und Zielfunktion definiert")
    ...
    def add_constraints(self):
        ...
        for i in range(n_teams):
            for k in range(n_courses):
                ...
                self.prob += (
                    pulp.lpSum([self.x[(i, j, k)] for j in range(n_teams)]) == 1,
                    constraint_name
                )
            ...
            for ii in range(i+1, n_teams):
                for j in range(n_teams):
                    for k in range(n_courses):
                        ...
                        self.prob += (
                            self.x[(i, j, k)] + self.x[(ii, j, k)] <= 1 + self.y[(i, ii, j, k)],
                            constraint_name
                        )
            ...
            for k in range(n_courses - 1):  # k -> k+1
                for j1 in range(n_teams):
                    for j2 in range(n_teams):
                        ...
                        distance = self.distances[(self.teams[j1].id, self.teams[j2].id)]
                        # Wenn Team i von Host j1 (Event k) zu Host j2 (Event k+1) geht
                        self.prob += (
                            distance * (self.x[(i, j1, k)] + self.x[(i, j2, k+1)] - 1) <= self.t[k],
                            constraint_name
                        )
    ...
    def solve(self) -> bool:
        """Löse das MIP-Problem"""
        logger.info("🚀 Starte MIP-Optimierung...")
        
        # Verwende Standard-Solver (meist CBC)
        self.prob.solve(pulp.PULP_CBC_CMD(msg=1))
        
        status = pulp.LpStatus[self.prob.status]
        logger.info(f"📊 Optimierungsstatus: {status}")
        ...
        if self.prob.status == pulp.LpStatusOptimal:
            ...
            logger.error(f"❌ Optimierung fehlgeschlagen: {status}")
            return False
            
    def extract_solution(self) -> Dict:
        """Extrahiere Lösung aus dem gelösten MIP-Model"""
        if self.prob.status != pulp.LpStatusOptimal:
            raise ValueError("Kein optimale Lösung verfügbar")
            ...
            for k, course in enumerate(self.courses):
                for j, host_team in enumerate(self.teams):
                    if pulp.value(self.x[(i, j, k)]) == 1:
                        ...
                        assignment['distances'][course] = self.distances[(team.id, host_team.id)]
                        
                        # Prüfe ob Team sich selbst hostet
                        if i == j:
                            assignment['course_hosted'] = course
            
            assignment['total_distance'] = sum(assignment['distances'].values())
        ...
        for k, course in enumerate(self.courses):
            ...
            for j, team in enumerate(self.teams):
                ...
                guest_count = sum([pulp.value(self.x[(i, j, k)]) for i in range(n_teams)]) 
        ...
        solution['penalties']['z1_violations'] = sum([pulp.value(self.z1[(j, k)]) for j in range(n_teams) for k in range(n_courses)])
        solution['penalties']['z2_violations'] = sum([pulp.value(self.z2[(j, k)]) for j in range(n_teams) for k in range(n_courses)])
        solution['penalties']['z3_violations'] = sum([pulp.value(self.z3[(i, ii)]) for i in range(n_teams) for ii in range(i+1, n_teams)])
        
        logger.info(f"✅ Lösung extrahiert: {len(solution['assignments'])} Team-Zuweisungen")
        return solution
    ...
    def optimize(self) -> Dict:
        ...
        try:
            ...
            # 5. Löse Problem
            if not self.solve():
                raise ValueError("Optimierung konnte keine Lösung finden")
